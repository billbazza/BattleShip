...
- Daily routine (exact order): load 00-Overview.md + AGENTS.md + learnings.md → review KPIs → prioritize by ROI → execute ONE subtask at a time → log every action in daily-log-YYYY-MM-DD.md → self-assess & course-correct → update learnings.md
...
- Memory hygiene: After any meaningful change, update relevant files in .Codex/memory/ (MEMORY.md = index only)

Respect this AGENTS.md strictly. Update it only when conventions genuinely change.