# Battleship — Carrd Website Copy (PLAIN TEXT)
Paste directly into Carrd. No markdown. Format using Carrd's toolbar.

---

## SECTION 1 — HERO
Background colour: #0a0a0a (dark)
Image: Midlife-fitness-reset.jpg — centred, ~200px wide

HEADLINE (large, Georgia, white):
12 weeks. Built around your life.
No excuses needed — you just need to commit.

SUBHEADLINE (smaller, #888888):
A reset for men over 40 who are done making excuses. No gym-bro nonsense. No crash diet. A system built for real life — starting with 12 weeks, built to last.

BUTTON (background #c41e3a, white text):
Take the Free Diagnosis
Link: https://form.typeform.com/to/wbD9VYUa

SMALL TEXT below button (#666666):
4 minutes. Personalised report sent same day.

---

## SECTION 2 — RECOGNISE YOURSELF?
Background colour: #f8f6f1 (off-white)

LABEL (tiny, uppercase, #c41e3a):
SOUND FAMILIAR?

BODY TEXT (#555555):
You're not unfit. You're undertreated.

You work hard. You show up for everyone else. Somewhere along the way your own health became the thing you'd sort out when things calm down.

Things haven't calmed down.

You're carrying weight you didn't used to carry. Getting out of breath doing things that never bothered you. Waking up tired despite sleeping. Joints that didn't used to talk back now have opinions.

You know something needs to change. You just haven't found an approach that fits real life.

Not a crash diet. Not a bootcamp. Not a 5am routine that lasts three weeks.

Something sustainable. Something that works around a job, a family, a commute, a life.

CLOSING LINE (slightly larger, #333333):
That's what Battleship is.

---

## SECTION 3 — WILL'S STORY
Background colour: #ffffff (white)
Image: IMG_2453.jpeg — right side or top of section

LABEL (tiny, uppercase, #c41e3a):
WHY THIS EXISTS

HEADLINE (#0a0a0a, Georgia):
I was exactly where you are.

BODY TEXT (#555555):
I used to think I was one of the fit ones. Biking, hiking, camping — I could keep up with anything. Then I hit my 40s and quietly stopped keeping up. Still told myself I was relatively fit. That was a lie I told myself for years.

After my 40th I bought a blood pressure monitor. Mild hypertension. My Apple Watch told me my fitness age was 55. I was in my early forties.

August 2024: a holiday photo of me getting out of the pool. I looked at it and thought: I'm a fat bastard. That was it. The final wake-up call.

I didn't hire a PT. I didn't join an expensive gym. I started walking — 20km every single day, without fail. Stopped drinking. Ate sensibly because I didn't want to undo the walks. Five months later I added gym work at lunchtime.

Five months of walking. Four months of gym. That was the entire formula.

Blood pressure: back to normal. Fitness age: 17. All the excess weight gone. Energy back. Showing up properly for my kids, my wife, my mates.

I work in technology. I love systems, tracking, and iterating. I built Battleship because I know the formula works — and because I know exactly how it feels to be where you are right now.

SIGN-OFF (#0a0a0a, Georgia, italic):
— Will, 47, Hampshire

---

## SECTION 4 — WHAT YOU GET
Background colour: #0a0a0a (dark)

LABEL (tiny, uppercase, #c41e3a):
THE 12-WEEK PROGRAMME

HEADLINE (white, Georgia):
Not a PDF. A proper system.

FEATURE ITEMS (white, one per line — use a list element in Carrd):
Your personalised plan, built from your intake form — not a template
Weekly check-ins with real responses. Adjustments made as you progress.
Weekly education emails: Zone 2, protein, sleep, fat loss science — the facts, not the hype
Calorie and nutrition guidance based on your weight and goal, not a generic chart
Training programme built around your injuries, your schedule, your gym access
Someone in your corner every week — for 12 weeks to start

SUBTEXT (#666666):
Starts with walking. Adds food awareness. Adds strength when you're ready. In that order. The first 12 weeks build the foundation. Most men don't want to stop there.

---

## SECTION 5 — PRICE + CTA
Background colour: #ffffff (white)

LABEL (tiny, uppercase, #c41e3a):
START HERE

HEADLINE (#0a0a0a, Georgia):
The free diagnosis is the first step.

BODY TEXT (#555555):
Fill in the intake form — 4 minutes, honest answers. I'll review it personally and send you a diagnosis specific to your history, your constraints, and your situation. Not a generic report.

If it resonates and you want to go further, the reset starts at £199 — or 3 payments of £75. That covers the first 12 weeks. Most men continue beyond that.

No pressure. No countdown timer. No lock-in.

BUTTON (background #c41e3a, white text):
Take the Free Diagnosis
Link: https://form.typeform.com/to/wbD9VYUa

SECONDARY LINK (small, #c41e3a, below button):
Already diagnosed and ready to start? Pay here.
Link: https://buy.stripe.com/3cI6oG79qefgb1CdhwejK00

TRUST LINE (#aaaaaa, small, centred):
7-day money-back guarantee   ·   will@battleshipreset.com   ·   I read every reply personally

---

## SECTION 6 — FOOTER
Background colour: #0a0a0a (dark)

TEXT (#555555, small, centred):
© 2026 Battleship – Midlife Fitness Reset   ·   battleshipreset.com
will@battleshipreset.com

TEXT (#444444, small, centred, italic):
Built for men who are done waiting.
